...
uv run python3 scripts/memory_budget.py \
  ...
  --kv-bits 8 \
  --pools 512 \
  > "$RESULTS_DIR/budget_8bit_kv.txt"
...
run_test() {
    ...
    if [[ "$name" == "kv_fp16" ]]; then
        budget_file="$RESULTS_DIR/budget_fp16_kv.txt"
    else
        budget_file="$RESULTS_DIR/budget_8bit_kv.txt"
    fi
...
echo "  Compare budget_fp16_kv vs budget_8bit_kv for KV memory savings"
echo "  Check actual snapshots to see if 8-bit KV closes the memory gap"
echo "  Verify if KV cache growth is the primary contributor to OOM"
