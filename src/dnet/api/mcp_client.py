...
class MCPToolClient:
    ...
    def _check_langchain_adapters(self) -> bool:
        ...
        try:
            ...
            if importlib.util.find_spec("langchain_mcp_adapters.client") is None:
                return False
            if importlib.util.find_spec("langchain_mcp_adapters.tools") is None:
                return False
    ...
    async def _load_tools_langchain(self) -> List[Any]:
        ...
        try:
            ...
            logger.info(
                f"✅ Loaded {len(self._langchain_tools)} tools from MCP servers"
            )
    ...
    async def _load_tools_direct(self) -> List[Any]:
        ...
        for name, config in self._server_configs.items():
            ...
            if transport == "stdio":
                ...
                logger.warning(
                    f"⚠️ Direct SDK doesn't support '{transport}' transport yet"
                )
...
async def create_mcp_client_from_presets(
    presets: List[str], additional_env: Optional[Dict[str, str]] = None
) -> MCPToolClient:
    """
    ...
    for preset_name in presets:
        ...
        if transport == "http":
            ...
            if preset.get("env_key"):
                env_value = (additional_env or {}).get(preset["env_key"]) or os.getenv(
                    preset["env_key"]
                )
                if env_value:
