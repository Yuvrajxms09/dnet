...
class DeepseekV32RingModel(BaseRingModel):
    ...
    def __init__(
        ...
        for i, layer in enumerate(sorted(assigned_layers or [])):
            self.layers.append(DeepseekV32DecoderLayer(config, layer))
    ...
    def apply_single_layer(
        ...
        if cache is not None and local_idx < len(cache):
            # DeepSeek V3.2 uses dual cache: [attention_cache, indexer_cache]
            c = cache[local_idx]
    ...
    def make_cache(self):
        """Create dual cache structure for DeepSeek V3.2 (attention + indexer caches)"""
