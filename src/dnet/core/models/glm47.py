...
from mlx_lm.models.glm4_moe import ModelArgs, DecoderLayer

from .base import BaseRingModel


class GLM47RingModel(BaseRingModel):
    """GLM47 model for ring topology inference.
    ...
    def __init__(
        ...
        for i, layer in enumerate(sorted(assigned_layers or [])):
            self.layers.append(DecoderLayer(config, layer))
    ...
    def sanitize(self, weights):
        # GLM-specific weight transformations (adapted from mlx_lm)
                ...
                for k in ["weight", "scales", "biases"]:
                    if f"{prefix}.mlp.experts.0.{m}.{k}" in weights:
                        to_join = [
                            ...
                            for e in range(getattr(self.config, 'n_routed_experts', 0))
                        ]
