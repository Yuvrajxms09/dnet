...
def test_structured_outputs_produces_valid_json():
    async def main():
        tok = FakeTokenizer()
        mm = FakeModelManager(tok)
        ad = FakeStrategyAdapter()
        ...
        req = ChatRequestModel(
            model="m",
            messages=[ChatMessage(role="user", content="Create a person")],
            max_tokens=30,
            structured_outputs=StructuredOutputsParams(json=schema),
        )
...
def test_structured_outputs_handles_generation_errors():
    async def main():
        tok = FakeTokenizer()
        mm = FakeModelManager(tok)
        ad = FakeStrategyAdapter()
        ...
        schema = {
            "type": "object",
            "properties": {"exact": {"type": "string", "enum": ["very_specific_phrase_only"]}},
            "required": ["exact"]
        }
        req = ChatRequestModel(
            model="m",
            messages=[ChatMessage(role="user", content="Say anything")],
            max_tokens=5,
            structured_outputs=StructuredOutputsParams(json=schema),
        )
